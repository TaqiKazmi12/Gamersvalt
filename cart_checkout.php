...
if (!isset($_SESSION['user_id']) || $_SERVER['REQUEST_METHOD'] !== 'POST' || !isset($_POST['selected_games'])) {
    header("Location: cart.php"); 
    exit();
}

$user_id = $_SESSION['user_id'];
$game_ids_to_checkout = $_POST['selected_games'];

if (empty($game_ids_to_checkout)) {
    header("Location: cart.php");
    exit();
}

$game_ids_to_checkout = array_map('intval', $game_ids_to_checkout);

$placeholders = implode(',', array_fill(0, count($game_ids_to_checkout), '?'));
$types = str_repeat('i', count($game_ids_to_checkout));

$sql = "SELECT id, title, price, thumbnail FROM games WHERE id IN ($placeholders)";
$stmt_games = $conn->prepare($sql);
$stmt_games->bind_param($types, ...$game_ids_to_checkout);
$stmt_games->execute();
$games_to_purchase = $stmt_games->get_result()->fetch_all(MYSQLI_ASSOC);
$stmt_games->close();

$total_price = array_sum(array_column($games_to_purchase, 'price'));
    ...
    <div class="checkout-wrapper">
        <div class="checkout-container">
            <div class="order-summary-panel">
                <h2>Order Summary</h2>
                <?php foreach ($games_to_purchase as $game): ?>
            ...
            <div class="payment-panel">
                ...
                <form action="process_cart_checkout.php" method="POST" id="payment-form">
                    <?php foreach ($games_to_purchase as $game): ?>
                        <input type="hidden" name="game_ids[]" value="<?php echo $game['id']; ?>">
                    <?php endforeach; ?>
